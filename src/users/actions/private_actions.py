...
async def _create_new_user(
    ...
    async with session.begin():
        ...
        email_user = await user_dal.get_user_by_email(email=body.email)
        phone_user = await user_dal.get_user_by_phone(phone=body.phone)
        if email_user:
            raise HTTPException(
        ...
        return PrivateDetailUserResponseModel(**user.__dict__)
